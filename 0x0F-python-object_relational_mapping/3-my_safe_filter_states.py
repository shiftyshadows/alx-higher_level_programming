#!/usr/bin/python3
"""This module defines a function that takes in arguments and displays
   all values in the states table of hbtn_0e_0_usa where name matches the
   argument, whilst enforcing safety from MySQL injections!
"""
import MySQLdb
import sys
...
def search_states_by_name(username, password, database_name, state_name):
    """
    Displays all values in the states table where the name matches the
    provided state name (safe from SQL injection).
    ...
    try:
        ...
        cursor = db.cursor()
        # Execute the SQL query with a placeholder to prevent SQL injection
